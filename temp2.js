...
function App() {
  ...
  const handleEdit = (e, id) => {
    ...
    setTodo(t[0].todo)
    setDescription(t[0].description) // Added
  ...
  const handleTodoChange = (e) => { // Modified
    setTodo(e.target.value)
  }

  const handleDescriptionChange = (e) => { // Added
    setDescription(e.target.value)
  }

  const handleAdd = () => {
    setTodos([...todos, { id: uuidv4(), todo, description, isCompleted: false, timestamp: new Date().toLocaleString() }])
    setTodo("")
      ...
      <div className='flex justify-center '>
        <div className="w-[50vw] bg-purple-400 min-h-screen m-8 p-10 rounded-lg ">
          ...
          <div className='gap-4 flex'>
            <input onChange={handleTodoChange} value={todo} type="text" className='w-[35vw] rounded-xl p-1' placeholder="Todo" /> {/* Modified */}
            <input onChange={handleDescriptionChange} value={description} type="text" className='w-[35vw] rounded-xl p-1' placeholder="Description" /> {/* Added */}
          ...
          <div className='flex flex-col gap-3'>
            ...
            {filteredTodos.map(item => {
              return (showFinished || !item.isCompleted) && (
                <div key={item.id} className='flex flex-col w-[40vw]'> {/* Modified */}
                  <div className='flex justify-between'>
                    ...
                    <div className='flex gap-2'>
                      ...
                      <button onClick={() => toggleExpand(item.id)} className='bg-violet-700 rounded-md w-auto p-1 text-white hover:bg-violet-500  hover:border'>{expandedTodoId === item.id ? 'Collapse' : 'Expand'}</button> {/* Added */}
                    </div>
                  </div>
                  {expandedTodoId === item.id && ( // Added
                    <div className='mt-2'> {/* Added */}
                      <p>Description: {item.description}</p> {/* Added */}
                      <p>Last updated: {item.timestamp}</p> {/* Added */}
                    </div> {/* Added */}
                  )} {/* Added */}
                </div>
              )
