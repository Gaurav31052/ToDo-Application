...
function App() {
  ...
  const handleAdd =(e)=>{ 
    ...
    axios.post('http://localhost:3000/todos',{ id: uuidv4(), todo, isCompleted: false, description, timestamp: new Date().toLocaleString() })
    .then(response=>setTodos(response.data))
    .catch(err=>console.log(err))
    setTodo("")
    setDescription("") 
  ...
  const filteredTodos = todos.find(item => item.todo.toLowerCase().includes(searchTerm.toLowerCase())); 
      ...
      <div className='flex justify-center '>
        <div className="w-[50vw] bg-purple-400 min-h-screen m-8 p-10 rounded-lg ">
          ...
          <div className='text-xl font-bold m-3'>
           ...
           {Todo.length === 0 && <div>No Todos</div>}
