...
function App() {
  ...
  const [searchTerm, setSearchTerm] = useState("") // Added line
  ...
  const handleSearch = (e) => { // Added line
    setSearchTerm(e.target.value) // Added line
  }

  const filteredTodos = todos.filter(item => item.todo.toLowerCase().includes(searchTerm.toLowerCase())); // Added line
      ...
      <div className='flex justify-center '>
        <div className="w-[50vw] bg-purple-400 min-h-screen m-8 p-10 rounded-lg ">
          ...
          <div className='gap-4 flex'>
            ...
            <button onClick={handleAdd} disabled={todo.length < 3} className='bg-violet-700 rounded-md w-20 text-white'>Save</button>
          </div>
          ...
          <div className='flex flex-col gap-3'>
            <input onChange={handleSearch} value={searchTerm} type="text" placeholder="Search todos..." className='w-[35vw] rounded-xl p-1 my-3' /> {/* Added line */}

            {filteredTodos.length === 0 && <div>No Todos</div>} {/* Modified line */}

            {filteredTodos.map(item => { // Modified line
