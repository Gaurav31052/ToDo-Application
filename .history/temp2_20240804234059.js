...
function App() {
  ...
  const [expandedTodoId, setExpandedTodoId] = useState(null) // Added
  ...
  const handleAdd =()=>{ // Modified
    setTodos([...todos, {id: uuidv4(), todo, isCompleted: false, description: "", timestamp: new Date().toLocaleString()}]) // Modified
    setTodo("")
    saveToLS(); // Added
  }
  ...
  const toggleExpand = (id) => { // Added
    setExpandedTodoId(expandedTodoId === id ? null : id); // Added
  }
